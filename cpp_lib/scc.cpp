...
// ll djcstra(const vector<VPII> &connect, int s, ll vr, ll vt) {
//     const int n = connect.size();
//     int l, r;
//     ll ret{0};
//     VL shortest(n, infint);
//     priority_queue<pair<ll, int>> pq;
//     pq.push(make_pair(0, s));
//     shortest[s] = 0;
//     while (!pq.empty()) {
...
// const long double myu = 1e-9;
// int getlattice(long double x, long double y, long double r, int xi) {
//     long double dx = abs((long double)xi - x), dy, yp, ym;
//     ll ret;
//     if (dx > r)
//         ret = 0;
//     else {
...
//         ym = y - dy - myu;
//         // cout << VD{yp, ym};
//         // cout << VI{(int)floor(yp), (int)ceil(ym)};
//         ret = (ll)floor(yp) - (ll)ceil(ym) + 1;
//     }
//     return ret;
// }

void post_order_dfs(int &counter, int u, graph &original_graph,
                    VPII &post_order) {
    for (auto [v, _] : original_graph.nodes[u].connect)
        if (!original_graph.nodes[v].check) {
            original_graph.nodes[v].check = true;
            post_order_dfs(counter, v, original_graph, post_order);
        }
    post_order.emplace_back(counter, u);
    ++counter;
}

void scc_dfs(int counter, int u, graph &transpose_graph) {
    transpose_graph.nodes[u].scc = counter;
    for (auto [v, _] : transpose_graph.nodes[u].connect)
        if (transpose_graph.nodes[v].scc == -1) {
            scc_dfs(counter, v, transpose_graph);
        }
}
...
int main() {
    ...
    int n, m, k, counter{0}, a, b;
    cin >> n >> m;
    graph original_graph(n), transpose_graph(n);
    VPII post_order;
    rep(i, m) {
        cin >> a >> b;
        a--, b--;
        original_graph.nodes[a].connect.emplace_back(b, 0);
        transpose_graph.nodes[b].connect.emplace_back(a, 0);
    }
    rep(i, n) if (!original_graph.nodes[i].check) {
        original_graph.nodes[i].check = true;
        post_order_dfs(counter, i, original_graph, post_order);
    }

    counter = 0;
    sort(allpt_r(post_order));

    for (auto [_, i] : post_order)
        if (transpose_graph.nodes[i].scc == -1) {
            scc_dfs(counter, i, transpose_graph);
            counter++;
        }
    // rep(i, n) cout << transpose_graph.nodes[i].scc << wsp;
    // cout << rt;
    k = counter;
    VI indeg(k, 0);
    vector<set<int>> scc_node(k);

    rep(i, n) for (auto [j, _] : original_graph.nodes[i].connect) {
        auto new_i = transpose_graph.nodes[i].scc;
        auto new_j = transpose_graph.nodes[j].scc;
        // cout << (VI){i, new_i, j, new_j};
        if (new_i != new_j) {
            scc_node[new_i].insert(new_j);
        }
    }

    rep(i, k) for (auto j : scc_node[i]) indeg[j]++;
    cout << count_if(allpt_c(indeg), [](int x) { return x == 0; }) << rt;
